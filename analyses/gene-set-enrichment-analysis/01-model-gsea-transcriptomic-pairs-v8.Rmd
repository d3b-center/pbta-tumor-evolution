...
metadata_with_gsva <- metadata %>%
  ...
  mutate(RNA_library_sum = case_when(grepl("exome_capture|poly-A|poly-A stranded", RNA_library) ~ "merged-RNA_library",
                                     TRUE ~ RNA_library),
...
bs_unique <- metadata_with_gsva %>%
  group_by(Kids_First_Biospecimen_ID, RNA_library_sum) %>%
  select(Kids_First_Biospecimen_ID, RNA_library_sum) %>% 
  distinct(Kids_First_Biospecimen_ID, .keep_all = TRUE)

print(table(bs_unique$RNA_library_sum))
print(table(metadata_with_gsva$RNA_library_sum))
