...
metadata_with_gsva_filtered <- readr::read_tsv(metadata_with_gsva_filtered_file, guess_max = 100000) %>% 
  ...
  mutate(bs_td_id = paste(Kids_First_Biospecimen_ID, tumor_descriptor, sep = "_"))
...
# Count #samples/library/timepoint: bs_td_id
metadata_with_gsva_filtered %>%
  
  #https://stackoverflow.com/questions/45198738/difference-between-distinct-vs-unique
  distinct(bs_td_id, .keep_all = TRUE) %>% 
  
  group_by(Kids_First_Participant_ID) %>%
  summarise(#RNA_library_sum = paste(sort(RNA_library), collapse = ", "),
            tumor_descriptor_sum = str_c(tumor_descriptor, collapse = ";")) %>%
  filter(!tumor_descriptor_sum %in% c(timepoints)) %>% 
  left_join(metadata_with_gsva_filtered) %>%
  distinct(bs_td_id, .keep_all = TRUE) %>% 
  #select(Kids_First_Participant_ID, bs_td_id, tumor_descriptor) %>% 
...
# Count #samples/library/timepoint: Kids_First_Participant_ID
#samples %>% 
...
common <- metadata_with_gsva_filtered %>% 
  filter(!RNA_library == "poly-A") %>% 
  select(Kids_First_Participant_ID, RNA_library, tumor_descriptor) %>% 
  unique() %>% 
  group_by(Kids_First_Participant_ID) %>%
  summarize(RNA_library_sum = paste(sort(RNA_library), collapse = ", "),
            tumor_descriptor_sum = str_c(tumor_descriptor, collapse = ";")) %>% 
  filter(!RNA_library_sum %in% c("stranded", "poly-A stranded", 
                                 "stranded, stranded",
                                 "stranded, stranded, stranded",
                                 "poly-A stranded, poly-A stranded",
                                 "poly-A stranded, poly-A stranded, poly-A stranded"),
         !tumor_descriptor_sum %in% c(timepoints)) %>% 
  left_join(metadata_with_gsva_filtered) %>% 
  #distinct(bs_td_id, .keep_all = TRUE) %>% 
  select(Kids_First_Participant_ID, bs_td_id, aliquot_id, RNA_library, RNA_library_sum)
  

aliquots <- print(length(unique(common$aliquot_id)))
samples <- print(length(unique(common$Kids_First_Participant_ID)))
bs_samples <- print(length(unique(common$bs_td_id)))
...
mapping_df %>%
  unique() %>% 
  #distinct(bs_td_id, .keep_all = TRUE) %>% #unique() %>%
  count(RNA_library, tumor_descriptor) %>%
  pivot_wider(names_from = tumor_descriptor, values_from = n) %>% 
  regulartable() %>%
  fontsize(size = 12, part = "all")
