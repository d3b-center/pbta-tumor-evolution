...
histology_label_mapping <- readr::read_tsv(broad_histology_palette_file, guess_max = 100000, show_col_types = FALSE) %>% 
  filter(cancer_group %in% c("Atypical Teratoid Rhabdoid Tumor", 
...
``` {r prepare-annotation-gsva-scores}
...
```{r, make-heatmap-timepoint, fig.width = 8, fig.height = 5, fig.fullwidth = TRUE, echo = TRUE}
...
pdf(gsva_timepoint_pdf, width = 8, height = 5)
...
# Heatmap by cancer group and timepoint

```{r, make-heatmap-timepoint-cg, fig.width = 8, fig.height = 5, fig.fullwidth = TRUE, echo = TRUE}

# or by cg and timepoint
gsva_annotation_df_cg <- gsva_annotation_df[gsva_ordered_bsids, ] %>%
  select(#-contains("broad_histology"),  # Drop extraneous columns
         #-tumor_descriptor_order,
         -cg_order,
         #-Kids_First_Biospecimen_ID,
  ...
  mutate(Cancer_group = factor(Cancer_group),
         Cancer_group = fct_relevel(Cancer_group, cg_f)) %>% 
  arrange(Cancer_group)


# Annotation bar intended for the top of the heatmap
column_heatmap_annotation <- HeatmapAnnotation(
  df = as.data.frame(gsva_annotation_df_cg),
  name = c("Cancer Group", "Timepoint"),
  col = list("Cancer_group" = annotation_colors_cg,
             "Timepoint" = annotation_colors_time),
  na_col = na_color$hex_codes,
...
pdf(gsva_cg_timepoint_pdf, width = 8, height = 5)
draw(gsva_heatmap, heatmap_legend_side = "right")
dev.off()
```
