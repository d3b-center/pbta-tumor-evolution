...
```
Rscript -e "rmarkdown::render('03-model-gsea-genomic-transcriptomic-pairs.Rmd', clean = TRUE)" 
```
...
matched_paired_assays_file <- file.path(files_dir, "paired_assays_matched_time_points.tsv") # file from add-sample-distribution module
...
plots_dir <-
  file.path(analysis_dir, "plots", "paired-genomic-transcriptomic")
...
results_dir <-
  file.path(analysis_dir, "results", "paired-genomic-transcriptomic")
...
patient_list <- readr::read_tsv(matched_paired_assays_file, guess_max = 100000, show_col_types = FALSE) %>%
...
#rna_library_list
#[1] "exome_capture"  "polya_stranded" "stranded"       "polya"  

#rna_library_list = "exome_capture"
#rna_library_list = "polya_stranded"
#rna_library_list = "stranded"
#rna_library_list = "polya"
...
for(i in 1:length(rna_library_list)){
  ...
  ###### barplots ######################################################################
  ### tukey
  name <- paste0(plots_dir, "/", "gsva_tukey_", rna_library, "-tumor_descriptor_barplot.pdf")
  print(name)
  ...
  ### anova
  name <- paste0(plots_dir, "/", "gsva_anova_", rna_library, "-tumor_descriptor_barplot.pdf")
  print(name)
  ...
  ### tukey
  name <- paste0(plots_dir, "/", "gsva_tukey_", rna_library, "-tumor_descriptor_dotplot.pdf")
  print(name)
  p <- create_dotplot(df = tumor_descriptor_model_results[["tukey"]],
                      x_value = tumor_descriptor_model_results[["tukey"]]$hallmark_name,
                      y_value = tumor_descriptor_model_results[["tukey"]]$tukey_p_value,
                      title_value = "tukey_p_value",
                      rna_library = rna_library)
  pdf(file = name, width = 12, height = 10)
