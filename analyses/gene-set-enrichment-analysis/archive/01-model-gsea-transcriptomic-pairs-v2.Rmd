...
for(i in 1:length(rna_library_list)){
  ...
  if(tumor_descriptor_n>=2){
    ...
    for(t in seq_along(td_models)){
      ...
      df$path_diff[df$pathway_score_difference > 0 & df$tukey_p_value < 0.05] <- "UP"
      # if pathway_score_difference < 0 and tukey_p_value < 0.05, set as "DOWN"
      df$path_diff[df$pathway_score_difference < 0 & df$tukey_p_value < 0.05] <- "DOWN"
      # Create labels
...
pathways <- df$hallmark_name
p1 <- plotGSEA(df, plot_type = "volcano", show_pathway = 3)
p2 <- plotGSEA(df, plot_type = "volcano", show_pathway = pathways)
p1 + p2 + plot_annotation(tag_levels = "A")
      

p <- plotEnrich(df, plot_type = "upset",main_text_size = 15,legend_text_size = 8)
