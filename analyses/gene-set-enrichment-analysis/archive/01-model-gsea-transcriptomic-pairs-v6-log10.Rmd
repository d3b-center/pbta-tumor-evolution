...
volcano_plots_dir <-
  file.path(plots_dir, "volcano_plots_p_value_log10")
if (!dir.exists(volcano_plots_dir)) {
  dir.create(volcano_plots_dir)
}


enhanced_volcano_plots_dir <-
  file.path(plots_dir, "enhanced_volcano_plots_p_value_log10")
if (!dir.exists(enhanced_volcano_plots_dir)) {
  dir.create(enhanced_volcano_plots_dir)
}
...
for(i in 1:length(rna_library_list)){
  ...
  if(tumor_descriptor_n>=2){
    ...
    for(t in seq_along(td_models)){
      ...
      df$log10_p_value <- (-log10(df$tukey_p_value))
      df$log2_pathway_score_difference <- log2(abs(df$pathway_score_difference))
      ...
      #signif_p_value <- -log10(0.05)
      p_value <- df$tukey_p_value
      signif_p_value <- 0.05 
      
      # if pathway_score_difference > 0  
      df$path_diff[df$pathway_score_difference > 0 & df$tukey_p_value < signif_p_value] <- "top_ranked_data"
      df$path_diff[df$pathway_score_difference > 0 & df$tukey_p_value > signif_p_value] <- "Not significant"

      # if pathway_score_difference < 0 
      df$path_diff[df$pathway_score_difference < 0 & df$tukey_p_value < signif_p_value] <- "bottom_ranked_data"
      df$path_diff[df$pathway_score_difference < 0 & df$tukey_p_value > signif_p_value] <- "Not significant"
      # Create labels
      df$labels <- with(df, ifelse(df$tukey_p_value < signif_p_value, hallmark_name_id, ""))
      
      # set order for labels
      df$path_diff <- factor(x = df$path_diff, levels = c("bottom_ranked_data", "Not significant", "top_ranked_data"))

    
      volcano_name <- paste0(volcano_plots_dir, "/", "gsva_tukey_", rna_library_list[i], "-", td_models[t], "-tumor_descriptor_volcano_plot.pdf")
      print(volcano_name)
      p <- create_volcano_plot_log10(df = df,
                                     x_value = df$log2_pathway_score_difference,
                                     y_value = df$log10_p_value,
                                     title_value = "tukey_p_value",
                                     rna_library = rna_library_list[i],
                                     td_model_id = td_models[t])
      ...
      volcano_name <- paste0(enhanced_volcano_plots_dir, "/", "gsva_tukey_", rna_library_list[i], "-", td_models[t], "-tumor_descriptor_enhanced_volcano_plot.pdf")
      print(volcano_name)
      p <- EnhancedVolcano(df, 
                           x = "log2_pathway_score_difference", 
                           y = "log10_p_value", 
                           lab = df$labels, 
                           title = paste0(td_models[t], " Enhanced Volcano plot for ", rna_library_list[i])
