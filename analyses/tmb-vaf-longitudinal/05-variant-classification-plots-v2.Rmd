...
if (!dir.exists(kids_PR_plots)) {
  dir.create(kids_PR_plots)
}
...
if (!dir.exists(molecular_subtype_PR_plots)) {
  dir.create(molecular_subtype_PR_plots)
}
...
molecular_subtype_PBTA_plots_dir <-
  file.path(PR_plots_dir, "molecular_subtype_PBTA_plots")
if (!dir.exists(molecular_subtype_PBTA_plots)) {
  dir.create(molecular_subtype_PBTA_plots)
}
...
```{r plot-molecular-subtype-PR, fig.width = 12, fig.height = 15, fig.fullwidth = TRUE}
PR_panel_df <- PR_count_df %>% 
...
```{r plot-molecular-subtype-PBTA, fig.width = 15, fig.height = 28, fig.fullwidth = TRUE}
PBTA_panel_df <- PBTA_count_df %>% 
  mutate(cg_id = str_replace_all(cg_id, " ", "_"),
         molecular_subtype = str_wrap(molecular_subtype, 12)) # Edit to fit in the plot title for the panel
...
for (i in seq_along(sample)){
  ...
  # Run function
  fname <- paste0(kids_PR_plots, "/", sample[i], "_Alteration_type_timepoints_kids_barplots_PR.pdf")
  print(fname)
