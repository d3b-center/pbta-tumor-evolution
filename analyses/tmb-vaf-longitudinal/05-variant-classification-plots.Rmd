...
count_df <- df_plot %>% 
  group_by(tumor_descriptor, cg_id, Variant_Classification) %>% 
  dplyr::count(cg_id)
...
Alteration_type <-count_df$Variant_Classification
```

# Alterations per timepoint

```{r plot-timepoint, fig.width = 5, fig.height = 5, fig.fullwidth = TRUE}
...
df_plot_cgGFAC <- df_plot %>%
  ...
  group_by(tumor_descriptor, cgGFAC, timepoint_cgGFAC_n, Variant_Classification) %>% 
...
for (i in seq_along(cgGFAC_id)){
  ...
  # Define parameters for function
  x_value <- df_plot_cgGFAC$timepoint_cgGFAC_n
  title <- paste(cgGFAC_id[i])
  
  # Run function
  p <- create_stacked_barplot_variant(count_df = df_plot_cgGFAC, x = x_value, palette = palette, title = title)
...
```{r plot-timepoint-model, fig.width = 15, fig.height = 6, fig.fullwidth = TRUE}
tm_df_plot <- df_plot %>%
  filter(!is.na(timepoints_models)) %>% 
  group_by(tumor_descriptor, cg_id, timepoints_models, Variant_Classification) %>% 
  dplyr::count(timepoint_cgGFAC_n)
...
for (i in seq_along(tm)){
  ...
  df_sub <- tm_df_plot %>%
   ...
   # Define parameters for function
  x_value <- tm_df_plot$tumor_descriptor
  title <- paste(tm[i])
