...
pbta_df <- readr::read_tsv(pbta_file, guess_max = 100000, show_col_types = FALSE) %>% 
  ...
  mutate(td_abbrev = case_when(grepl("Diagnosis", tumor_descriptor) ~ "Dx",
                               ...
                               grepl("Unavailable", tumor_descriptor) ~ "Un",
                              TRUE ~ tumor_descriptor),
         molecular_subtype_edited = str_replace(molecular_subtype, "^\\S* ", ""),
         td_ms_plot_group = paste(td_abbrev, molecular_subtype_edited, sep = ":"))
...
no_samples <- print(length(unique(PL_df$Kids_First_Participant_ID)))

# Let's count number of samples 
PL_count_df <- PL_df %>% 
  group_by(tumor_descriptor, td_ms_plot_group, cg_id, molecular_subtype, Kids_First_Participant_ID, match_id, Variant_Classification) %>% 
  dplyr::count(cg_id) %>% 
