...
timepoints_order_abbrev <- c("Dx", "Pro", "Rec", "Dec", "SM", "Unavail")
...
pbta_df <- readr::read_tsv(pbta_file, guess_max = 100000, show_col_types = FALSE) %>% 
  ...
  mutate(tumor_descriptor = case_when(grepl("Diagnosis", tumor_descriptor) ~ "Dx",
         ...
         td_ms_plot_group = paste(tumor_descriptor, molecular_subtype_edited, sep = ":"))
...
PBTA_count_df <- PBTA_df %>% 
  left_join(PBTA_td_plot_group_n_df) %>% 
