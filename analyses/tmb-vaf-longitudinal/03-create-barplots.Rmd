...
plot_df <- tmb_df_filter %>% 
  dplyr:::mutate(tumor_descriptor = factor(tumor_descriptor),
                 ...
                 tumor_descriptor = fct_reorder(tumor_descriptor, descriptors)) 


 mutate(eye_color = fct_reorder(eye_color, mean_mass)) %>%
   
mutate(tumor_descriptor = fct_reorder(tumor_descriptor, descriptors))
  mutate(Source = fct_relevel(Source, "Modified InterVar", "ClinVar"))
