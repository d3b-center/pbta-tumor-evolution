...
```{r create-barplot-cancer-type, fig.width = 18, fig.height = 10, fig.fullwidth = TRUE}
...
for (i in seq_along(cancer_groups)) {
  ...
  if (i == 1) {
    ...
    # Run function
    p <- create_barplot_ct(tmb_df = df_ct_sub, 
                                 ylim = ylim, 
...
```{r create-barplot-cancer-type-log2_tmb, fig.width = 18, fig.height = 10, fig.fullwidth = TRUE}
df_ct <- tmb_genomic_filter %>% 
  arrange(patient_id, tumor_descriptor_order, descriptors) %>% 
  # create plot order 
  mutate(plot_order = row_number())
...
for (i in seq_along(cancer_groups)) {
  ...
  if (i == 1) {
    ...
    # Name plots
    fname <- paste0(plots_dir, "/", "log2_tmb-genomic", "-", cancer_groups[i], ".pdf")
    print(fname)
    
    # Run function
    p <- create_barplot_ct_log2_tmb(tmb_df = df_ct_sub, 
                                 ylim = ylim, 
                                 ct_id = cancer_groups[i])
    pdf(file = fname, width = 18, height = 10)
    print(p)
...
# TMB dumbbell plot across timepoints and cancer types 
...
# Total number of mutations dumbbell plot across timepoints and cancer types 
...
# log2mut dumbbell plot across timepoints and cancer types 
...
for (i in seq_along(cancer_groups)) {
  ...
  if (i == 1) {
    ...
    # Name plots
    fname <- paste0(plots_dir, "/", "log2mut-genomic-dumbbell", "-", cancer_groups[i], ".pdf")
    print(fname)
    
    # Run function
    p <- create_dumbbell_ct_log2mut(tmb_df = df_ct_sub, 
                                 ylim = ylim, 
                                 ct_id = cancer_groups[i])
    pdf(file = fname, width = 18, height = 10)
    print(p)
...
Here, we want to explore the number of mutations per timepoint and biospecimen sample per patient case by creating barplots.
