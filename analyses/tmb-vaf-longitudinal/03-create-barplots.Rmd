...
dev.off()
```
We notice that there are samples with high TMB (hypermutant samples). Next, we will exclude these samples (threshold >= 1,000 mutations) from further analysis in this notebook. Attention is needed in cases with high number of mutations in only one timepoint. We will also remove those so we always have matched longitudinal samples.
...
```{r create-stacked-barplot-filter-cancer-group-sum, fig.width = 12, fig.height = 8, fig.fullwidth = TRUE}
...
for (i in seq_along(cancer_groups)) {
  ...
  # Run function
  fname <- paste0(plots_dir, "/", cancer_groups[i], "-", "Stacked-barplot-tmb-genomic-filter.pdf")
  print(fname)
...
# Number of mutations per `Kids_First_Participant_ID` and `Kids_First_Biospecimen_ID`
...
yvalue = unique(as.character(tmb_sub$experimental_strategy))
print(yvalue)
