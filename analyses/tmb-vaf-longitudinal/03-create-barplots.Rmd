...
tmb_genomic_filter <- tmb_genomic %>%
  ...
  mutate(cancer_group_sum = ifelse(short_histology == "HGAT", "High-grade glioma",
         ...
         tumor_descriptor_order = case_when(grepl("Diagnosis", tumor_descriptor) ~ "1",
...
Howmany <- length(unique(tmb_genomic_filter$cancer_group_sum))
plot_lst <- vector(mode='list', length = Howmany) #' an empty list
...
for (i in 1:Howmany) {
  ...
  if (i == 1) {
    ...
    plot_lst[[i]] <- p  # add each plot into plot list
...
# Set plot layout
layout.matrix <- matrix(c(2, 1, 0, 3), ncol = 3)
