...
maf_df <- readr::read_tsv(maf_file, guess_max = 100000, show_col_types = FALSE) %>%
  select(Kids_First_Biospecimen_ID, Chromosome, Start_Position,
         Reference_Allele, Tumor_Seq_Allele2, t_ref_count, t_alt_count, tmb) %>% 
  filter(!tmb >= 10) %>% # Remove hypermutants
  
  # There are alterations with high number of read counts.
  # We will exclude those with >1000 for now.
  filter(!t_alt_count >= 1000,
         !t_ref_count >= 1000) %>% 
...
genomic_paired_df <- readr::read_tsv(genomic_paired_file, guess_max = 100000, show_col_types = FALSE) %>% 
  ...
  select(Kids_First_Participant_ID, Kids_First_Biospecimen_ID, sample_id, 
         aliquot_id, tumor_descriptor, descriptors, timepoints_models, cg_multiple, 
         cg_id, cgGFAC, cg_id_kids, td_bs_id)

genomic_paired_samples <- print(length(unique(genomic_paired_df$cg_id_kids)))
...
# kids_specimens <- print(length(unique(kids_specimens_n_df$Kids_First_Participant_ID)))

df <- kids_specimens_n_df %>% 
