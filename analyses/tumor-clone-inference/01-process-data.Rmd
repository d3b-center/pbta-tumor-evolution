...
create_df <- function(df, samples) {
  # Reference df
    ...
    # Merge df
    merge_df <- ref_df %>% 
      full_join(alt_df, by = c("cg_id_kids", "Chr", "Position", "Wild", "Mut")) %>% 
      as.data.frame() 
...
for (x in seq_along(cg_id_kids_names)){
  ...
  df_sub <- df %>%
    ...
    select("cg_id_kids", "td_bs_id", "Chr", "Position", "Wild", "Mut", "XX_ref", "XX_alt")
  ...
  for (i in seq_along(td_bs_id_names)) {
    ...
    # Print df per pair
    m <- create_df(df = df_sub, samples = td_bs_id_names[i])
  
    # Assign name to df and store all df in list
    df_name <- paste(td_bs_id_names[i])
    df <- assign(df_name, m)
    df_list[[df_name]] <- df
