...
# write function to run for the list of dataframes
df_ref <- function(x){
  data_all[[i]]$df_ref %>%
  subset(select = c(match_id_unique, Kids_First_Biospecimen_ID_descriptor, XX.ref)) 
  pivot_wider(data_all[[i]]$df_ref, names_from = Kids_First_Biospecimen_ID_descriptor, values_from = XX.ref)
}

# run function
data_all[[i]]$df_ref<- lapply(data_all, df_ref)

#####################
# we will next create a data frame for the read counts for the alt variant
# XX:alt
# create unique identifier based on chromosome and chromosomal position for each variant
# let's also keep the information with tumor sample and time point together. This is useful for visualization purposes later.

data_all[[i]]$df_alt <-  data_all[[i]] %>%
       mutate(match_id_unique = paste(Chr, Position, sep = "_")) %>%
       mutate(Kids_First_Biospecimen_ID_descriptor = paste(Kids_First_Biospecimen_ID, tumor_descriptor, sep = "_")) %>%
...
df_alt <- function(x){
  ...
  pivot_wider(data_all[[i]]$df_alt, names_from = Kids_First_Biospecimen_ID_descriptor, values_from = XX.alt)
}

# run function
data_all[[i]]$df_alt<- lapply(data_all, df_alt)
...
data_all[[i]]$merge_read_counts <- list(data_all[[i]]$df_ref, left_join(data_all[[i]]$df_alt)) %>% 
