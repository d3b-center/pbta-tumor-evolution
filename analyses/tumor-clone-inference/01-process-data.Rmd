...
for (x in seq_along(cg_id_kids_names)){
  ...
  for (i in seq_along(td_bs_id_names)) {
    ...
    cg_kids_df <- power_full_join(df_list, by = c("cg_id_kids", "mut_id", "Chr", "Position", "Wild", "Mut")) %>% 
      replace(is.na(.), 0) %>% 
      write_tsv(file.path(CloneFinder_results_dir, paste0(name, ".txt")))
...
samples_path_df <- df_temp %>%
  select(cg_id_kids) %>% 
  unique() %>% 
  mutate(path = CloneFinder_results_dir,
         additional = 1) %>% 
      write_tsv(file.path(results_dir, "samples_path.tsv"))
```  
