...
PT_Z4BF2NSB_df <- readr::read_tsv(nautilus_dec_file, guess_max = 100000, show_col_types = FALSE) %>%
  ...
  #dplyr::mutate(sample_id = str_replace(sample_id, ":", ""),
   #             sample_id = str_replace(sample_id, "_", "")) %>% 
...
goi_df <- readr::read_tsv(goi_list_file, guess_max = 100000, show_col_types = FALSE) %>% 
  select(Kids_First_Participant_ID, Timepoint_bs_id, gene_protein, sym) %>% 
...
clonevol_df <- PT_Z4BF2NSB_df %>% 
  ...
  #mutate_if(is.character, as.numeric(ref.count, var.count, vaf, ccf, cluster)) %>% 
  # TEST
  dplyr::mutate(#cluster = case_when(grepl("Deceased:BS_2T4EJ6KN", sample_id) ~ "2",
                                    #TRUE ~ "1"),
    
                # `infer.clonal.models` function doesn't accept 0 for clustering
                cluster = case_when(grepl("0", cluster) ~ "1"),
                ccf = (ccf*100),
                vaf = (vaf*100)#,
...
df <- clonevol_df %>%
  ...
  mutate(id = paste(tumor_descriptor, gene, "ccf", sep = ".")) %>% 
  pivot_wider(names_from = "id", values_from = c("ccf"), values_fill = 0) %>% #values_fn = list
  
  
  #tidyr::spread(sample_id, c("ref.count", "var.count", "vaf", "ccf")) %>%
  #mutate_all( ~replace(., lengths(.)==0, NA)) %>%
  as.data.frame() %>% 
  #replace(is.na(.), 0) %>% 
  #mutate(cluster = as.numeric(cluster)) %>%
  #mutate_if(is.character, as.numeric()) %>% 
  select(-c(Kids_First_Participant_ID, unique_id, sample_id, tumor_descriptor)) 


count_col <- ncol(df)

df <- df %>%mutate_at(c(2:count_col), as.numeric) %>% 
    write_tsv(file.path(clonevol_input_dir, "clonevol_input2.tsv"))
