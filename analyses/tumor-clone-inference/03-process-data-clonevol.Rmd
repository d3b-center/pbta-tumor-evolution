...
cluster_df <- readr::read_tsv(pyclonevi_output_file, guess_max = 100000, show_col_types = FALSE) %>% 
  ...
  mutate(cluster = case_when(grepl("0", cluster) ~ "1"))  # NEED TO MAKE IT REPRODUCIBLE THIS for a number of clusters!!
...
clonevol_df <- PT_Z4BF2NSB_df %>% 
  ...
  mutate(cluster = case_when(grepl("Deceased:BS_2T4EJ6KN", sample_id) ~ "2"))  # TEST
...
df <- clonevol_df %>%
  ...
  pivot_wider(names_from = "sample_id", values_from = c("ref.count", "var.count", "vaf", "ccf")) %>% 
  as.data.frame()
