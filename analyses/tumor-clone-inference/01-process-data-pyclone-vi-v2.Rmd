...
pyclonevi_input_dir <-
  file.path(analysis_dir, "results", "pyclonevi-input")
...
genomic_paired_df <- readr::read_tsv(genomic_paired_file, guess_max = 100000, show_col_types = FALSE) %>% 
  ...
  dplyr::mutate(mutation_id = paste(Kids_First_Participant_ID, Chromosome, Start_Position, Reference_Allele, Tumor_Seq_Allele2, sep = ":"),
...
pyclone_input <- data_list_bind %>% 
 ...
 # write.table(file.path(pyclonevi_input_dir, paste0("PT_Z4BF2NSB-v2.tsv")), 
  #                sep = "\t", 
    #              row.names = FALSE, 
     #             quote = FALSE, 
       #           fileEncoding = "UTF-8")
