...
colnames(data_list_bind)[1] <- "mutation_id" 
...
clonevol_df <- patient_df %>% 
  select(Kids_First_Participant_ID, tumor_descriptor, Kids_First_Biospecimen_ID, sample_id, Hugo_Symbol, t_ref_count, t_alt_count,
         VAF, CCF, Chromosome, Start_Position, Reference_Allele, Tumor_Seq_Allele2) %>% 
  dplyr::mutate(mutation_id = paste(Kids_First_Participant_ID, tumor_descriptor, Kids_First_Biospecimen_ID, Chromosome, Start_Position, Reference_Allele, Tumor_Seq_Allele2, sep = ":")) %>% 
