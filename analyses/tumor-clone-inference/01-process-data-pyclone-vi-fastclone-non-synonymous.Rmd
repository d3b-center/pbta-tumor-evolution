...
pyclonevi_input_dir <-
  file.path(analysis_dir, "results", "pyclone-vi-input-non-synonymous")
...
# File path to plot directory
pyclone_plots_dir <-
  file.path(analysis_dir, "plots", "pyclone-vi-non-synonymous")
...
#print(table(laml@data$Variant_Classification))
...
pyclone_input <- data_list_bind %>% 
  ...
  right_join(pyclone_all_samples_df) %>% 
...
pyclone_input_all <- nautilus_dec_df %>%
  ...
  write_tsv(file.path(results_dir, "pyclone_input_all_data_non_synonymous.tsv"))
...
# To add plot with #bs/kids_id

```{r}
df_file <- file.path(results_dir, "samples_eligible_for_phylogeny_non_synonymous.tsv")
df <- readr::read_tsv(df_file, guess_max = 100000, show_col_types = FALSE) 
...
# Plot stacked barplot 
p <- print(ggplot(count_df , aes(x = cg_id_kids, 
                          y = n, 
             ...
             scale_y_discrete(breaks = scales::pretty_breaks(n = 6)) +
             labs(title = paste("Number of bs per cg_id_kids", sep = " ")) + 
             labs(x = "", y = "# bs"))
  
# Save the plot
ggsave(filename = "Number_bs_per_cg_id_kids.pdf", 
       path = plots_dir, 
       width = 12, 
       height = 8,
       device = "pdf", 
