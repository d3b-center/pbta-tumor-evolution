...
snv_vaf_ccf_subset <- subset(snv_vaf_ccf_formula, 
  ...
  write_tsv(file.path(results_dir, "snv_vaf_ccf_subset.tsv"))
...
# we will use Tumor_Seq_Allele1 for now (to repeat analysis with Tumor_Seq_Allele2)
...
read_counts_all_samples <- setnames(read_counts_all_samples, old = c('Chromosome','Start_Position','Reference_Allele', "Tumor_Seq_Allele1"), 
         new = c('Chr','Position','Wild', "Mut")) %>%
  write_tsv(file.path(results_dir, "read_counts_all_samples.tsv"))


# Create dir to save read count data to be used as input for CloneFinder
temp_path <-
  file.path(results_dir, "CloneFinder_input_read_count")
if (!dir.exists(temp_path)) {
  dir.create(temp_path)
}


# we will split Read_count data by the "Kids_First_Biospecimen_ID" column
#samples <- read_counts_all_samples$Kids_First_Biospecimen_ID
  
#for(i in unique(samples)) {
#        nam <- paste("read_counts_all_samples", i, sep = ".")
#        assign(nam, read_counts_all_samples[data_names==i,])
#        }


read_counts_all_samples %>% 
  group_split(Kids_First_Biospecimen_ID) %>% 
  walk(~write_csv(.x, paste0(temp_path, .x$Kids_First_Biospecimen_ID[1], ".csv")))
